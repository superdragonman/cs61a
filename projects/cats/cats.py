...
from Demos.print_desktop import print_processor
...
def pick(paragraphs, select, k):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 1
    qualified_list = [i for i in paragraphs if select(i)]
    if len(qualified_list) - 1 < k:
        return ''
    else:
        return qualified_list[k]
...
def about(keywords):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def is_have(paragraph):
        para_list = split(lower(remove_punctuation(paragraph)))
        tr_fa = [i for i in para_list for j in keywords if i == j]
        if not tr_fa:
            return False
        return True
    return is_have
...
def accuracy(typed, source):
    ...
    if len(source_words) == 0 and len(typed_words) == 0:
        ...
        for i in range(len(source_words)):
            if typed_words[i] == source_words[i]:
                number += 1
    return number / len(typed_words) * 100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for _ in word_list:
        if _ == typed_word:
            return _
    correct = [x for x in word_list if diff_function(typed_word, x, limit) <= limit]
    if len(correct) == 0:
        return typed_word
    else:
        correct_diff = [diff_function(typed_word, x, limit) for x in correct]
        min_index = min(enumerate(correct_diff), key = lambda x: x[1])[0]
        return correct[min_index]
    # END PROBLEM 5
...
def time_per_word(words, timestamps_per_player):
    ...
    for player in tpp:
        players_time = [player[i] - player[i - 1] for i in range(1, len(player))]
        times.append(players_time)
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for word_index in word_indices:
        word_times = [times[player][word_index] for player in player_indices]
