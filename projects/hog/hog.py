...
def roll_dice(num_rolls, dice=six_sided):
    ...
    count = 0
    rolled_one = False
    for number in range(num_rolls):
        num = dice()
        if num == 1:
            rolled_one = True
        else:
            count += num
    return 1 if rolled_one else count
    # END PROBLEM 1
...
def boar_brawl(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    bit = player_score % 10
    ten_bit = opponent_score // 10 % 10
    return max(1, 3 * abs(bit - ten_bit))
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for number in range(goal):
        for number2 in range(goal):
            if strategy(number, number2) ==roll:
                continue
            else:
                return False
    return True
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for i in range(10):
        mylist.append(average(i + 1, dice))
    value = mylist[0]
    index = 0
    for i in range(1, 10):
        if mylist[i] > value:
