...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.stock > 0 and self.balance >= self.price:
            ...
            left_balance = self.balance - self.price
            self.balance = 0
            return f'Here is your {self.product} and ${left_balance} change.' if left_balance > 0 else f'Here is your {self.product}.'
        elif self.stock == 0:
...
def two_list(vals, counts):
    ...
    "*** YOUR CODE HERE ***"
    result = Link.empty
    for i in range(len(vals) - 1, -1, -1):  # 从后往前插入，方便构造链表
